<?php
$FM_VERS = "8.03";		// script version

/* ex:set ts=4 sw=4 et:
 * FormMail PHP script from Tectite.com.  This script requires PHP 4 or later.
 * Copyright (c) 2001-2007 Root Software and Open Concepts Pty Ltd
 ...
 * Last Modified: RR 12:26 Wed 15 August 2007
 * QVCS Version: $Revision: 1.122 $
 *
 * Read This First
 * ~~~~~~~~~~~~~~~
...
$TARGET_EMAIL = array("^youremail@gvsu\.edu$");

			/* Help: http://www.tectite.com/fmdoc/def_alert.php */
define("DEF_ALERT","youremail@gvsu.edu");
...
function LoadBuiltinLanguage()
{
	...
	$aMessages[MSG_THANKS_PAGE] = 'Thanks!  We\'ve received your information '.
								  'and, if it\'s appropriate, we\'ll be contacting you in 8-9 months...if you want it faster, send money';
...
function DetectMimeAttack($a_fields,&$s_attack,&$s_info)
{
        ...
        //
    $a_rec_flds = array("recipients","cc","bcc","subject");   // all these fields must be scalar (not arrays)
...
function DetectSpecialsAttack($a_fields,&$s_attack,&$s_info)
{
    ...
    //
function DetectManyURLsAttack($a_fields,&$s_attack,&$s_info)
{
...
list($aFieldOrder,$aCleanedValues,$aRawDataValues) = ParseInput($aAllRawValues);
    ...
    //
if (isset($aSessionVars["FormList"]) && $SPECIAL_VALUES["multi_start"] != 1)
{
...
if (isset($SPECIAL_VALUES["imgverify"]) && !empty($SPECIAL_VALUES["imgverify"]))
{
	...
	if (!isset($aSessionVars["VerifyImgString"]) &&
        !isset($aSessionVars["turing_string"]))
    ...
    else
    {
	    if (strtoupper(str_replace(" ","",$SPECIAL_VALUES["imgverify"])) !==
				    strtoupper($aSessionVars["turing_string"]))
		    UserError("img_verify",GetMessage(MSG_VERIFY_MATCH)/*,
                        "imgverify='".$SPECIAL_VALUES["imgverify"]."' ".
                        "turing_string='".$aSessionVars["turing_string"]."'"*/);
    }
}
...
if (isset($SPECIAL_VALUES["arverify"]) && !empty($SPECIAL_VALUES["arverify"]))
	if (IsAROptionSet('HTMLTemplate') || IsAROptionSet('PlainTemplate'))
	{
		...
		else
		{
            ...
            {
	            if (strtoupper(str_replace(" ","",$SPECIAL_VALUES["arverify"])) ===
				            strtoupper($aSessionVars["turing_string"]))
			        $bARImgMatch = true;
            }
            if (!$bARImgMatch)
